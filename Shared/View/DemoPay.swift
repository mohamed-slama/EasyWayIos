...
public extension iPaymentButton {
    ...
    @available(iOS 11.0, *)
    public static func applePayDemo(price: Double) {
        ExampleApplePayPopup().pay(price: price)
    }
}
...
//    public func startPayment(completion: @escaping PaymentCompletionHandler) {
    public func pay(price: Double) {
        completionHandler = demoCompletion
        
        let subtotal = PKPaymentSummaryItem(label: "Subtotal", amount: NSDecimalNumber(value: price), type: .final)
        ...
        let total = PKPaymentSummaryItem(label: "Demo Corp", amount: NSDecimalNumber(value: price+8.04 ), type: .final)
